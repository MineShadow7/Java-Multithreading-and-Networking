...
public class ClientController {
    ...
    @FXML
    private ListView<GamePlayer> playerListView;
    @FXML
    ...
    private final ClientGame client = new ClientGame();


    private final ObservableList<GamePlayer> playersList = FXCollections.observableArrayList();

    public ClientController() {
    }

    public void initialize() {
        renderer = new ClientRenderer(fieldPane);
        playerListView.setCellFactory((ListView<GamePlayer> view) -> {
            return new ClientPlayerStatsList();
        });
        playerListView.setItems(playersList);
        ClientApplication.addStopHandler(() -> {
            if (gameRunner != null) {
                ...
                while (true) {
                    while (true) {
                        synchronized (client) {
                            ...
                            List<GamePlayer> gamePlayers = client.getRoom().getPlayers();
                            Platform.runLater(() -> {
                                playersList.setAll(gamePlayers);
                            });
                        }
    ...
    public void showLeaderboard() {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(ClientController.class.getResource("/com/example/demo1/leaderboard-dialog.fxml"));
            fxmlLoader.setController(new ClientLeaderboard(client));
