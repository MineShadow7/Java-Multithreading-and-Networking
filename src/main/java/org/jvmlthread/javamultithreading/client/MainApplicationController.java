...
public class MainApplicationController {
    ...
    @FXML
    private ListView<GamePlayerClass> playerListView;
    @FXML
    ...
    private final ClientGameClass client = new ClientGameClass();


    private final ObservableList<GamePlayerClass> playersList = FXCollections.observableArrayList();
    ...
    public void initialize() {
        ...
        playerListView.setCellFactory((ListView<GamePlayerClass> view) -> {
            return new PlayerStatisticsList();
        });
        playerListView.setItems(playersList);
        MainApplication.addStopHandler(() -> {
            if (gameRunner != null) {
                ...
                while (true) {
                    while (true) {
                        synchronized (client) {
                            ...
                            List<GamePlayerClass> gamePlayerClasses = client.getRoom().getPlayers();
                            Platform.runLater(() -> {
                                playersList.setAll(gamePlayerClasses);
                            });
                        }
